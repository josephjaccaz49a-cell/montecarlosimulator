...
if st.button("🎬 Lancer la simulation"):
    ...
    # (2) Réel
    ax = axes[1]
    ax.fill_between(dates, q10_nom.values, q90_nom.values, alpha=0.20, 
                    label="Fourchette probable (80 % des cas)")
    ax.plot(dates, q50_nom.values, label="Scénario central (50/50)")
    ax.plot(dates, q10_nom.values, linestyle="--", linewidth=1, 
            label="Scénario défavorable (90 % de chances d’être au-dessus)")
    ax.plot(dates, q90_nom.values, linestyle="--", linewidth=1, 
